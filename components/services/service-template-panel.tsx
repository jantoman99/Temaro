...
import { StarterServicesForm } from "@/components/services/starter-services-form";
import { getStarterServiceTemplates, type ServiceTemplate } from "@/lib/service-templates";
...
export function ServiceTemplatePanel({
  ...
  const starterTemplates = getStarterServiceTemplates(industry);
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_320px]">
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-1 block text-xs font-medium leading-5 text-muted-foreground">
                    {template.durationMinutes} min · {template.price} Kč · pauza {template.bufferMinutes} min
                  </span>
                </Link>
              );
            })}
        ...
        <StarterServicesForm templates={starterTemplates} />
      </div>
    </section>
  );
}
