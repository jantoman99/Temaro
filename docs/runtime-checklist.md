...
- Runtime po aktuálním landing redesignu ověřit na staging `/`: hero text `Rezervace drží den pohromadě.`, class `temaro-time-page`, hero canvas `Provozní stůl dne`, `operating-table-stage`, `Dnes 08:00-18:00`, segmenty `Kadeřnictví / Barber / Kosmetika / Trenér`, desktop navigaci `Produkt / Ceník / Ukázka / Návody`, mobilní hamburger menu, sekce `jak-to-funguje`, `bento`, `produktove-demo`, `cenik`, `bezpecnost` a nulový horizontální overflow na desktopu i mobilu. Sekce `scenare`, `trust-bar`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` už nemají existovat.
...
- Historická poznámka k dispatch passu: `Živý dispečink dne`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` byly nahrazené operating-table směrem a už nejsou aktivní runtime očekávání.
...
- Runtime po operating-table passu ověřit na staging `/`: hero obsahuje `Provozní stůl dne`, `Rezervace drží den pohromadě.`, `Dnes 08:00-18:00`, `Volné okno chráněné`, `Rizikový klient`, `Jeden den, žádné přepínání`, `Nastavit stejný den` a funkční tlačítko `Rezervovat 11:00`. Samostatný `dispatch-proof-strip`, `dispatch-day-rail`, `mobile-day-rail`, `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem` a `id="trust-bar"` už se nesmí vrátit.
- Runtime po operating-table passu ověřit viewporty 1440, 1366, 768, 390 a 375px: žádný horizontální overflow; desktop má inbox zpráv + kalendář + rozhodnutí ve first foldu, tablet schovává `operations-inbox`, mobil ukazuje produktový výběr času a rozhodnutí dne bez horizontálního scrollu.
...
- Lokální ověření 2026-06-11 14:08 CEST po operating-table passu: `npx vitest run tests/landing-polish.test.ts` prošel 29/29, `npx impeccable detect app components` je čistý, `npm run check` prošel s 607 Vitest testy, migrations check, type-check, lint a produkční build. Lokální Playwright screenshoty jsou bez horizontálního overflow: `output/playwright/landing-redesign-2026-06-11/implementation-pass-3/home-1440-pass3.png`, `home-1366-pass3.png`, `home-768-pass3.png`, `home-390-pass3.png`, `home-375-pass3.png`.
