...
- Runtime po aktuálním product-window landing redesignu ověřit na staging `/`: hero text `Rezervace, které vidíte hned v kalendáři.`, class `product-window-hero`, app rám `product-window-frame`, klientský telefon `booking-phone-preview`, CTA `Začít zdarma` a `Spustit produktovou ukázku`, produktové plochy `Přehled provozu`, `Týmový kalendář`, `Rezervační stránka`, `Zákaznický účet`, desktop navigaci `Produkt / Ceník / Ukázka / Návody`, mobilní hamburger menu, sekce `casovy-engine` jako `product-proof-gallery product-proof-board`, `jak-to-funguje`, `bento`, `produktove-demo`, `cenik`, `bezpecnost` a nulový horizontální overflow na desktopu i mobilu. Sekce `scenare`, `trust-bar`, `photo-led-hero`, `salon-phone-card`, `salon-scenes-grid`, `operating-table-stage`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` už nemají existovat.
...
- Historická poznámka k dispatch passu: `Živý dispečink dne`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` byly postupně nahrazené a v aktuálním product-window směru už nejsou aktivní runtime očekávání.
...
- Runtime po product-window passu ověřit viewporty 1440, 1366, 768, 390 a 375px: žádný horizontální overflow; desktop má výrazný text vlevo a app frame s absolutním booking telefonem vpravo, tablet ukazuje produktový frame bez telefonu, mobil má produktový frame hned po CTA a proof deska nesmí vytvářet horizontální scroll.
...
- Lokální ověření 2026-06-11 16:48 CEST po product-window passu: `npx vitest run tests/landing-polish.test.ts` prošel 30/30, `npx impeccable detect app components` je čistý, `npm run check` prošel s 608 Vitest testy, migrations check, type-check, lint a produkční build. Lokální Playwright screenshoty jsou bez horizontálního overflow: `output/playwright/landing-product-window-2026-06-11/pass6/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png` a `metrics.json`.
- Staging ověření 2026-06-11 17:01 CEST po product-window passu: preview deploy `dpl_8HRQBqTehprzprf6p4zroA66NPjB` je aliasovaný na `https://rezervacni-system-dev.vercel.app`; `/api/health` je `ok`, HTML obsahuje `Rezervace, které vidíte hned v kalendáři`, `product-window-hero`, `product-window-frame`, `booking-phone-preview`, `product-proof-gallery`, `product-proof-board`, `Co klient a tým skutečně uvidí` a neobsahuje `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-*.webp`, `operating-table-stage`, `Provozní stůl dne`, `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`.
