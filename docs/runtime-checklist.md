...
- Staging ověření pro Time Atelier pass zatím čeká na commit/push/deploy. Po nasazení ověřit `/api/health=ok`, HTML signatury `Váš kalendář má mít vlastní rytmus`, `Ateliér času Temaro`, `temaro-time-atelier-hero`, `temaro-cinema-frame`, `temaro-screenshot-gallery`, `/marketing/product/temaro-product-app-screen.jpg` a nepřítomnost `salon-operating-hero`, `salon-command-frame`, `product-window-hero`, `product-window-frame`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip`, `Den před Temarem`, `Den s Temarem` ani `id="trust-bar"`.
