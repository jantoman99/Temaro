...
- Staging reapply po upřesnění uživatele 2026-06-15 13:51 CEST: velký produktový hero je znovu aplikovaný commitem `ca23ff4` a dev alias `https://rezervacni-system-dev.vercel.app` míří na preview `dpl_HRrucNWTuXahPWkJ1v8gmJvrHSis`. `/api/health=ok`, HTML obsahuje `temaro-fresha-product-hero`, `temaro-product-glow`, `temaro-product-desktop`, `temaro-product-phone` a neobsahuje `temaro-black-hero-grid`; desktop/mobile `overflowX=0`.
