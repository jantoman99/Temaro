...
- Runtime po WOW passu ověřit na staging `/`: hero obsahuje `Den před Temarem`, `Den s Temarem`, `Tři zprávy čekají`, `Volné okno`, `Riziko no-show`, `Potvrzeno online` a `Rezervovat 11:00`; samostatný `dispatch-proof-strip`, `dispatch-day-rail`, `mobile-day-rail` a `id="trust-bar"` už se nesmí vrátit.
- Runtime po WOW passu ověřit viewporty 1440, 1366, 768, 390 a 375px: žádný horizontální overflow, tablet používá kompaktní before/after panel místo plného chaos sloupce, mobil ukazuje produktový výběr času v prvním scrollu a desktop drží split-screen chaos vs. kalendář.
...
- Lokální ověření 2026-06-11 13:22 CEST po WOW passu: `npm run check` prošlo s 607 Vitest testy, migrations check, type-check, lint a produkční build. `npx impeccable detect app components` je čistý. Lokální Playwright screenshoty: `output/playwright/landing-wow-2026-06-11/home-1440-final3.png`, `home-1366-final3.png`, `home-768-final3.png`, `home-390-final3.png`, `home-375-final3.png`.
