...
- Staging rollback po nevhodném Fresha large product hero passu 2026-06-15 13:41 CEST: `https://rezervacni-system-dev.vercel.app` míří na ruční preview deploy `dpl_9rjeo2ZnBEz2T1LcqM3J3YiWgdGR` po revert commitech `8ae3d1c` a `8fe2ba1`. `/api/health` vrací `status=ok`, `env.ok=true`, `supabase.ok=true`, `rate_limit.configured=false`. HTML obsahuje `temaro-black-hero-grid`, `temaro-hero-photo-stack`, `temaro-hero-platform-card` a neobsahuje `temaro-fresha-product-hero`, `temaro-fresha-product-shell`, `temaro-product-glow`, `id="produktovy-pribeh"`, `FeatureStoryBento` ani `temaro-salon-wall`. Lokální ověření rollbacku: landing guard 22/22 a `npm run check` prošlo s 600 Vitest testy, migrations check, type-check, lint a produkční build.
