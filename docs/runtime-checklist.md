...
- Lokální all-public article pass 2026-06-15 22:32 CEST: všech 8 veřejných článkových rout (`/rezervacni-system-pro-barbery`, `/rezervacni-system-pro-kadernictvi`, `/rezervacni-system-pro-kosmeticky-salon`, `/rezervacni-system-pro-masaze`, `/rezervacni-system-pro-wellness`, `/jak-snizit-no-show`, `/sms-pripominky-rezervaci`, `/rezervacni-system-bez-marketplace-provizi`) používá jednotný article systém s `temaro-seo-article`, `temaro-article-hero`, `temaro-article-photo`, `temaro-article-product-card`, `temaro-article-mini-nav`, `temaro-article-product-proof` a `<details>` FAQ. Interní mini záložky jsou nativní `<a>` kotvy, aby se po opakovaném klikání netvořil slepený hash `#pravidla#produkt`. Staré `signal-hero signal-grid` article routy se nesmí vrátit. Marketing navigace z public subpages musí používat root kotvy `/#produkt`, `/#cenik`, `/#bez-marketplace`, `/#bezpecnost`; klik na `Bezpečnost` z článků má skončit na `/#bezpecnost`, ne na lokálním hashi subpage. Ověření: landing guard 25/25, public Playwright `all public article routes|marketing nav links|segment SEO|business marketing navigation|landing page exposes` 5/5 včetně kliků na mini záložky `#kroky`, `#pravidla`, `#produkt`, `#faq` a plný `npm run check` s 613 Vitest testy, migrations check, type-check, lint a produkční build.
- Staging all-public article pass 2026-06-15 22:38 CEST: `https://rezervacni-system-dev.vercel.app` míří na preview deploy `dpl_XdbNVyn3P7vajpbLBRFyXsns5cVz` z commitu `6846c0c`. `/api/health` vrací `status=ok`, `env.ok=true`, `supabase.ok=true`, `rate_limit.configured=false`. HTML všech 8 článkových rout obsahuje `temaro-seo-article`, `temaro-article-mini-nav`, `temaro-article-product-card` a neobsahuje `signal-hero signal-grid`; staging Playwright `all public article routes|marketing nav links|segment SEO|business marketing navigation|landing page exposes` prošel 5/5.
- Production deploy 2026-06-15 22:53 CEST: `https://rezervacni-system-xi.vercel.app` míří na Vercel deploy `dpl_2Ud5hSuLVqqQgSMZtEn3HHzqptAy` z commitu `9daded4`. Produkční `/api/health` vrací `status=ok`, `env.ok=true`, `env.missing=0`, `supabase.configured=true`, `supabase.ok=true`, `rate_limit.configured=false`. HTML homepage obsahuje `Plnější kalendář`; všech 8 článkových rout obsahuje `temaro-seo-article`, `temaro-article-product-card` a neobsahuje `signal-hero signal-grid`; produkční Playwright `all public article routes|marketing nav links|segment SEO|business marketing navigation|landing page exposes` prošel 5/5. Poznámka pro další deploy: nepoužívat prebuilt production deploy bez následné health kontroly, první prebuilt pokus `dpl_Bai6zgdpD4zMxrKbxydreSKDApLw` měl `status=degraded`; standardní `npx vercel --prod --yes` health opravil.
- Staging SEO article redesign 2026-06-15 21:47 CEST: `https://rezervacni-system-dev.vercel.app` míří na preview deploy `dpl_4urk7q4CN7Dyc1E5T92X4oXsZuLV` z commitu `8a842c4`. `/api/health` vrací `status=ok`, `env.ok=true`, `supabase.ok=true`, `rate_limit.configured=false`; staging Playwright `landing page exposes|segment SEO|marketing nav links` prošel 3/3. Staging audit `output/playwright/seo-article-redesign-2026-06-15/staging/` potvrdil u tří SEO článků `imageCount=2`, `viewportImages=1`, `productCards=1`, `miniNav=1`, `details=5`, `oldSignalArticle=false`, `overflowX=0`; klik `Bezpečnost` z článku skončil na `/#bezpecnost` za 380ms a `Produktový pohled` na `/#produkt` za 304ms. Console error je pouze známý Vercel Live feedback script blokovaný CSP.
...
- Staging public light-only pass 2026-06-15 20:58 CEST: `https://rezervacni-system-dev.vercel.app` míří na preview deploy `dpl_6cL6dBczJ5q35Nf7ki4vcQGq8uLF` z commitu `654f020`. `/api/health` vrací `status=ok`, `env.ok=true`, `supabase.ok=true`, `rate_limit.configured=false`; staging public Playwright smoke prošel 6/6. Browser audit `output/playwright/public-light-pass-2026-06-15-body-guard/staging/` potvrdil na desktopu i mobilu `themeButtons=0`, `overflowX=0`, light tokeny a světlé `body`; jediná console hláška je Vercel Live feedback script blokovaný CSP.
...
- Aktuální produkce od 2026-06-15 22:53 CEST: `https://rezervacni-system-xi.vercel.app` míří na Vercel deploy `dpl_2Ud5hSuLVqqQgSMZtEn3HHzqptAy` z commitu `9daded4`. Runtime obsah má aktuální dev landing směr s hero copy `Plnější kalendář bez volání.`; starý rollback deploy `dpl_3iNgj8xzsVMwypVkkicgP8c4ohqD` už není aktivní produkce.
...
- Staging keep-alive 2026-06-16: `.github/workflows/staging-keepalive.yml` denně volá `https://rezervacni-system-dev.vercel.app/api/health`, aby free Supabase staging projekt `gzkurmputkhqdqgrlaje` měl pravidelnou read-only aktivitu. Workflow nepoužívá secrety, produkční URL ani deploy. Po e-mailovém upozornění Supabase byl ručně ověřen staging `/api/health=ok`, `supabase.ok=true`, latency `1603ms`. Před každým release dál ručně kontrolovat staging health; pro garantované nepausování stagingu je potřeba Supabase Pro. Poslední ověření 2026-06-16 14:00 CEST: keep-alive workflow test 3/3, focused Vitest 14/14, `git diff --check` čistý a plný `npm run check` prošel s 614 Vitest testy, migrations check, type-check, lint a produkční build.
