...
- Runtime po aktuálním salon-operating landing redesignu ověřit na staging `/`: hero text `Plnější kalendář bez marketplace provizí.`, class `salon-operating-hero`, app rám `salon-command-frame`, oborový přepínač `industry-switcher`, klientský telefon `booking-phone-preview` jen na desktopu, CTA `Registrovat salon` a `Spustit ukázku`, desktop navigaci `Produkt / Pro koho / Návody / Ceník / Bez marketplace / Bezpečnost / Ukázka`, mobilní hamburger menu, sekce `pro-koho`, `produktovy-dukaz`, `cenik`, `bez-marketplace`, `jak-to-funguje`, `bezpecnost` a nulový horizontální overflow na desktopu i mobilu. Sekce `produktove-demo`, `scenare`, `trust-bar`, `product-window-hero`, `product-window-frame`, `product-proof-gallery`, `photo-led-hero`, `salon-phone-card`, `salon-scenes-grid`, `operating-table-stage`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` už nemají existovat.
...
- Historická poznámka k dispatch passu: `Živý dispečink dne`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` byly postupně nahrazené a v aktuálním salon-operating směru už nejsou aktivní runtime očekávání.
...
- Runtime po salon-operating passu ověřit viewporty 1440, 1366, 768, 390 a 375px: žádný horizontální overflow; desktop má výrazný text vlevo a salonový app frame s absolutním booking telefonem vpravo, tablet ukazuje app frame bez telefonu, mobil má app frame hned po CTA a proof/compare panely nesmí vytvářet horizontální scroll.
...
- Lokální ověření 2026-06-11 18:55 CEST po salon-operating passu: `npx vitest run tests/landing-polish.test.ts` prošel 20/20, `npx impeccable detect app components` je čistý, `npm run lint` prošel a `npm run check` prošel s 598 Vitest testy, migrations check, type-check, lint a produkční build. Lokální Playwright screenshoty jsou bez horizontálního overflow: `output/playwright/landing-salon-operating-2026-06-11/final/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png` a `metrics.json`.
- Staging ověření 2026-06-11 23:51 CEST po salon-operating passu: preview deploy `dpl_8ozzwSbGcdG2XKRP9Ku3KJssKgFZ` je aliasovaný na `https://rezervacni-system-dev.vercel.app`; `/api/health` je `ok`, HTML obsahuje `Plnější kalendář bez marketplace provizí`, `salon-operating-hero`, `salon-command-frame`, `industry-switcher`, `marketplace-free-proof`, `salon-proof-board`, `marketplace-compare-panel` a neobsahuje `product-window-hero`, `product-window-frame`, `product-proof-gallery`, `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-*.webp`, `operating-table-stage`, `Provozní stůl dne`, `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`. Staging screenshot audit `output/playwright/landing-salon-operating-2026-06-11/staging/` potvrdil horizontální overflow `0` pro 1440, 1366, 768, 390 a 375px.
