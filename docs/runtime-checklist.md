...
- Lokální SEO article redesign 2026-06-15 21:39 CEST: `/jak-snizit-no-show`, `/sms-pripominky-rezervaci` a `/rezervacni-system-bez-marketplace-provizi` používají sdílený `SeoArticlePage` s `temaro-article-hero`, `temaro-article-photo`, `temaro-article-product-card`, `temaro-article-mini-nav`, `temaro-article-product-proof` a `<details>` FAQ. Staré `signal-hero signal-grid` article routy se nesmí vrátit. Marketing navigace z public subpages musí používat root kotvy `/#produkt`, `/#cenik`, `/#bez-marketplace`, `/#bezpecnost`; klik na `Bezpečnost` z `/jak-snizit-no-show` má skončit na `/#bezpecnost`, ne na `/jak-snizit-no-show#bezpecnost`. Ověření: landing guard 25/25, public Playwright `landing page exposes|segment SEO|marketing nav links` 3/3, browser audit `output/playwright/seo-article-redesign-2026-06-15/local-after/` pro desktop/mobile (`imageCount=2`, `viewportImages=1`, `details=5`, `overflowX=0`, console errors `0`) a plný `npm run check` s 613 Vitest testy, migrations check, type-check, lint a produkční build.
