...
- Runtime po aktuálním photo-led landing redesignu ověřit na staging `/`: hero text `Méně zvonění.` a `Plnější kalendář.`, class `photo-led-hero`, asset `salon-day-hero.webp`, overlay `salon-phone-card`, štítky `Volno dnes` a `Bez provize z vašich klientů`, segmenty `Kadeřnictví / Barber / Kosmetika / Trenér`, desktop navigaci `Produkt / Ceník / Ukázka / Návody`, mobilní hamburger menu, sekce `casovy-engine` jako `salon-proof-strip`, `salon-scenes-grid`, `jak-to-funguje`, `bento`, `produktove-demo`, `cenik`, `bezpecnost` a nulový horizontální overflow na desktopu i mobilu. Sekce `scenare`, `trust-bar`, `operating-table-stage`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` už nemají existovat.
...
- Historická poznámka k Time Engine vrstvě: staré `time-engine-salon.webp`, `time-engine-beauty.webp`, `time-engine-fitness.webp`, zprávy/sloty a scan efekt byly nahrazené photo-led sekcí `Provoz v obrazech`.
...
- Historická poznámka k dispatch passu: `Živý dispečink dne`, `dispatch-day-rail`, `mobile-day-rail` a `dispatch-proof-strip` byly postupně nahrazené a v aktuálním photo-led směru už nejsou aktivní runtime očekávání.
...
- Runtime po photo-led passu ověřit viewporty 1440, 1366, 768, 390 a 375px: žádný horizontální overflow; desktop má výrazný text vlevo a fotku s absolutním booking overlayem vpravo, tablet ukazuje fotku před mobilními CTA/proof bloky, mobil má fotku viditelnou nad kompaktním vodorovným overlayem a `photo-led-mobile-actions` až pod fotkou.
