...
- Na landing page zkontrolovat hero: claim `Plnější kalendář bez volání.`, CTA `Registrovat salon`, CTA `Vidět ukázku`, foto ribbon `Kadeřnictví / Barber / Beauty`, code-native produktový desktop+telefon náhled, kanály `Web podniku`, `Instagram`, `Google`, `QR recepce`, booking řádky `08:30`, `10:30`, `12:15`, `16:00`, detail `16:00 z Instagramu` a důkaz pod herem `0 %`, `4 kanály`, `SMS + záloha`, `CRM demo`.
...
- Ověřit, že hero nepoužívá `booking-phone-preview` ani screenshotový produktový `<img>`; produktový pohled je postavený z HTML/CSS prvků kalendáře. `next/image` je povolené jen pro kontrolovaný `temaro-hero-photo-ribbon` a oborové service fotky, ne pro screenshoty produktu nebo cizí Fresha assety.
...
- Na mobilní šířce ověřit, že hero foto ribbon, produktový telefon, `temaro-reservation-flow`, `temaro-operations-board` a `marketplace-compare-panel` nemají horizontální scroll a sticky CTA nepřekrývá důležitý obsah.
