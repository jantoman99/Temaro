...
- Na stagingu `https://rezervacni-system-dev.vercel.app` ověřit `/api/health=ok` a HTML signatury aktuálního landing směru: obsahuje `Plnější kalendář`, `temaro-hero-product-visual`, `temaro-product-desktop`, `temaro-product-phone`, `temaro-booking-scene`, `Fotky provozu musí vysvětlovat rezervace`, `Salon na fotce. Rezervace v kontextu.`, `temaro-reservation-flow`, `temaro-flow-board`, `xl:flex`, `xl:hidden`, `Zdroj → kalendář → klient → zpráva → platba`; neobsahuje `temaro-hero-photo-ribbon`, `temaro-hero-photo-card`, `HeroPhotoRibbon`, `heroPhotoCards`, `temaro-black-hero-grid`, `temaro-salon-wall`, `temaro-cinema-frame`, `temaro-screenshot-gallery`, `temaro-screenshot-strip`, `booking-phone-preview`, `/marketing/product/temaro-product-app-screen.jpg`, `/marketing/product/temaro-client-booking-mobile.jpg`, `/marketing/product/temaro-business-discovery.jpg`, `salon-operating-hero`, `salon-command-frame`, `product-window-hero`, `product-window-frame`, `product-proof-gallery`, `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-hero.webp`, `operating-table-stage`, `Provozní stůl dne`, `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`.
- Poslední staging ověření 2026-06-15 16:13 CEST splnilo předchozí bod na preview deployi `dpl_7WRT8bEaoFCJbHCRhd3zTsaC5uf5`, alias `https://rezervacni-system-dev.vercel.app`; staging screenshot audit je v `output/playwright/fresha-image-led-pass-2026-06-15/staging/`.
...
- Na landing page zkontrolovat Fresha visual cleanup: první dojem musí být konkrétní rezervační software pro salon/provoz s čistým bílým canvasem, klidnější typografií a větším produktem. Dekorativní mřížky a oranžové/mint fleky v pozadí se nesmí vrátit; grid může být jen součást kalendářového produktu.
- Pod hero proof bandem ověřit nový image-led blok `temaro-booking-scene`: vlevo má být výrazný ink copy panel, vpravo 3 lokální oborové fotky s rezervačním overlayem. Blok má působit jako kontext rezervací v reálném salonu, ne jako dekorativní galerie; na mobilu musí být bez horizontálního scrollu.
- Pod herem nesmí být samostatný `salon-operating-hero`, `salon-command-frame`, `product-window-hero`, `photo-led-hero`, `salon-phone-card`, `operating-table-stage`, `dispatch-proof-strip`, `dispatch-day-rail`, `mobile-day-rail`, `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem` ani `trust-bar`; navazovat má image-led booking scene, produktový průchod, oborová sekce, produktový důkaz, ceník a anti-marketplace porovnání.
...
- Ověřit, že hero nepoužívá `booking-phone-preview`, `next/image` ani screenshotový produktový `<img>`; produktový pohled je postavený z HTML/CSS prvků kalendáře. `next/image` je povolené jen pro navazující oborové service fotky mimo hero, ne pro screenshoty produktu nebo cizí Fresha assety.
...
- V sekci `Produkt v provozu` / `#produktovy-pribeh` ověřit code-native průchod `temaro-reservation-flow`: zdroj rezervace, kalendář, klient, SMS a záloha jsou vidět jako jeden tok, ne jako generické textové karty.
...
- Na mobilní šířce ověřit, že produktový telefon, `temaro-reservation-flow`, `temaro-operations-board` a `marketplace-compare-panel` nemají horizontální scroll a sticky CTA nepřekrývá důležitý obsah.
...
- Runtime OAuth config 2026-06-15 19:23 CEST: Supabase allow-list už obsahuje `https://rezervacni-system-dev.vercel.app/**` a Google provider je zapnutý. Facebook/Apple netestovat, dokud nejsou v Supabase doplněné jejich client ID/secret.
- Google OAuth 2026-06-15 19:59 CEST: na stagingu ručně dokončit login testovacím Google účtem. Pro `/register` očekávat redirect na `/register/complete`, až tam doplnit název podniku a jméno vlastníka, potom ověřit `/start`. Pro `/login` ověřit návrat do adminu existujícího ownera. Pro `/account/login` ověřit, že Google účet bez tenant metadata skončí jen v zákaznickém účtu.
