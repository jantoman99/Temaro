...
- Aktivní landing směr od 2026-06-11 15:24 CEST je `Salonový den, ne dashboard`: photo-led hero `Méně zvonění. Plnější kalendář.`, velká salonová fotka, kompaktní interaktivní booking overlay `salon-phone-card`, sekce `Provoz v obrazech` se třemi fotkami reálných služeb a jednodušší bento/product rytmus. Směr vychází z konkurenčního patternu více obrazové služby + klidná produktová vrstva; nevrací fake metriky, reference ani loga zákazníků.
- Nové marketing assety jsou v `public/marketing/salon-day-hero.webp`, `salon-day-barber.webp`, `salon-day-beauty.webp` a `salon-day-fitness.webp`. Původní AI výstupy zůstaly mimo repo v `/home/hanys/.codex/generated_images/019eb678-d270-7400-8813-a377bee91e6f/`.
- Lokální screenshot audit po photo-led passu: `output/playwright/landing-photo-led-2026-06-11/home-1440-fold.png`, `home-1366-fold.png`, `home-768-fold.png`, `home-390-fold.png`, `home-375-fold.png` a odpovídající `*-full.png`. Browser audit potvrdil horizontální overflow `0` pro 1440, 1366, 768, 390 a 375px; `salon-phone-card` má computed `position=absolute`, mobilní reservation list je kompaktní `flex` a stránka renderuje 4 marketing obrázky.
- Lokální ověření 2026-06-11 15:24 CEST po photo-led passu: `npx vitest run tests/landing-polish.test.ts` prošel 30/30, `npx impeccable detect app components` prošel bez výstupu a `npm run check` prošel s 608 Vitest testy, migrations check, type-check, lint a produkční build.
- Staging deploy 2026-06-11 15:39 CEST po photo-led passu: commit `e58e337` je na `origin/dev`, ručně nasazený přes `npx vercel build` + `npx vercel deploy --prebuilt --yes` jako preview `dpl_62Zw1AQqgNc7yP62BERA7ACycX3Z` a aliasovaný na `https://rezervacni-system-dev.vercel.app`. Ověřeno: `/api/health` je `ok`, HTML obsahuje `Méně zvonění`, `Plnější kalendář`, `photo-led-hero`, `salon-day-hero`, `salon-phone-card`, `salon-proof-strip` a neobsahuje `operating-table-stage`, `Provozní stůl dne`, `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`.
