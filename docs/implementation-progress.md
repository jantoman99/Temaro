...
- Aktivní landing směr od 2026-06-11 16:48 CEST je `Temaro Product Window`: first fold stojí na reálném produktovém browser rámu `product-window-frame`, klientském booking telefonu `booking-phone-preview`, headline `Rezervace, které vidíte hned v kalendáři.` a následné produktové desce `product-proof-board`. Landing už nepoužívá lifestyle/AI salonové fotky, `photo-led-hero`, `salon-phone-card` ani salon-day obrázky; všechny hlavní vizuály jsou code-native pohledy na app/booking UI.
- Photo-led assety `public/marketing/salon-day-*.webp` zůstávají historickým artefaktem pro případné oborové/archivní použití, ale root homepage je nepoužívá. Aktivní zákaz je nevracet `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip`, `id="trust-bar"`, before/after hero ani photo-led root hero.
- Lokální screenshot audit po product-window passu: `output/playwright/landing-product-window-2026-06-11/pass6/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png` a `metrics.json`. Browser audit potvrdil horizontální overflow `0` pro 1440, 1366, 768, 390 a 375px; `booking-phone-preview` má na desktopu computed `position=absolute`, `product-proof-board` je přítomný, staré photo/dispatch signatury jsou `false` a stránka renderuje `0` marketing `<img>`.
- Lokální ověření 2026-06-11 16:48 CEST po product-window passu: `npx vitest run tests/landing-polish.test.ts` prošel 30/30, `npx impeccable detect app components` prošel s exit 0 bez výstupu a `npm run check` prošel s 608 Vitest testy, migrations check, type-check, lint a produkční build.
- Staging deploy 2026-06-11 17:01 CEST po product-window passu: commit `3c8fd0c` je na `origin/dev`, ručně nasazený přes `npx vercel build` + `npx vercel deploy --prebuilt --yes` jako preview `dpl_8HRQBqTehprzprf6p4zroA66NPjB` a aliasovaný na `https://rezervacni-system-dev.vercel.app`. Ověřeno: `/api/health` je `ok`, HTML obsahuje `Rezervace, které vidíte hned v kalendáři`, `product-window-hero`, `product-window-frame`, `booking-phone-preview`, `product-proof-gallery`, `product-proof-board`, `Co klient a tým skutečně uvidí` a neobsahuje `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-*.webp`, `operating-table-stage`, `Provozní stůl dne`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`.
...
- Root landing page používá aktuální claim `Rezervace, které vidíte hned v kalendáři.`
- Landing má zkrácenou top navigaci `Produkt`, `Ceník`, `Ukázka`, `Návody`.
- Hero má product-window app frame s plochami `Přehled provozu`, `Týmový kalendář`, `Rezervační stránka` a `Zákaznický účet`; interaktivní průchod zůstává na `/ukazka`.
