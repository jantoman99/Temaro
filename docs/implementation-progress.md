...
Aktualizováno: 2026-06-11 18:55 CEST
...
- Staging deploy pro salon-operating pass zatím čeká na commit/push a ruční Vercel preview deploy. Poslední ověřený staging před touto změnou stále ukazuje product-window pass `dpl_8HRQBqTehprzprf6p4zroA66NPjB`; po deployi je nutné ověřit `/api/health` a HTML signatury `Plnější kalendář bez marketplace provizí`, `salon-operating-hero`, `salon-command-frame`, `industry-switcher`, `marketplace-free-proof`, `salon-proof-board`, `marketplace-compare-panel`.
