...
- Aktivní landing směr od 2026-06-14 14:52 CEST je `Temaro Time Atelier` / `calendar-led control room`: first fold používá claim `Váš kalendář má mít vlastní rytmus.`, badge `Ateliér času Temaro`, class `temaro-time-atelier-hero` a code-native CRM kalendář `temaro-control-room` s `temaro-calendar-board`, `temaro-live-booking`, `temaro-live-thread` a detailem vybraného signálu. Hero už nepoužívá `next/image`, screenshot cinema, telefonní overlay ani assety z `public/marketing/product/`.
