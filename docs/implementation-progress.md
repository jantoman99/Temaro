...
Aktualizováno: 2026-06-11 23:51 CEST
...
- Aktivní landing směr od 2026-06-11 18:55 CEST je `Temaro Salon Operating`: first fold používá headline `Plnější kalendář bez marketplace provizí.`, salonový app rám `salon-command-frame`, oborový přepínač `Barber/Kosmetika/Kadeřnictví/Masáže`, klientský `booking-phone-preview` na desktopu a jasné CTA `Registrovat salon`. Směr navazuje na audit Salony strukturou (obory, cena, salonový kontext), ale drží Temaro diferenciaci: vlastní klienti, bez marketplace provizí a app-only produktové pohledy.
- Root homepage už nepoužívá `product-window-hero`, `product-window-frame`, `product-proof-gallery`, lifestyle/AI salonové fotky, `photo-led-hero`, `salon-phone-card` ani salon-day obrázky. Aktivní zákaz zůstává nevracet `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip`, `id="trust-bar"`, before/after hero ani photo-led root hero. Photo-led assety `public/marketing/salon-day-*.webp` zůstávají jen historickým artefaktem mimo root homepage.
- Lokální screenshot audit po salon-operating passu: `output/playwright/landing-salon-operating-2026-06-11/final/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png` a `metrics.json`. Browser audit potvrdil horizontální overflow `0` pro 1440, 1366, 768, 390 a 375px; `salon-operating-hero=true`, `hasProductDemoSection=false`, staré dispatch signatury `false`, stránka renderuje `0` marketing `<img>` a mobilní `salon-command-frame` se po opravě vejde do viewportu (`358px` na 390px, `343px` na 375px).
- Lokální ověření 2026-06-11 18:55 CEST po salon-operating passu: `npx vitest run tests/landing-polish.test.ts` prošel 20/20, `npx impeccable detect app components` prošel s exit 0 bez výstupu, `npm run lint` prošel a `npm run check` prošel s 598 Vitest testy, migrations check, type-check, lint a produkční build.
- Staging deploy 2026-06-11 23:51 CEST po salon-operating passu: commit `f834477` je na `origin/dev`, ručně nasazený přes `npx vercel build` + `npx vercel deploy --prebuilt --yes` jako preview `dpl_8ozzwSbGcdG2XKRP9Ku3KJssKgFZ` a aliasovaný na `https://rezervacni-system-dev.vercel.app`. Ověřeno: `/api/health` je `ok`, HTML obsahuje `Plnější kalendář bez marketplace provizí`, `salon-operating-hero`, `salon-command-frame`, `industry-switcher`, `marketplace-free-proof`, `salon-proof-board`, `marketplace-compare-panel` a neobsahuje staré product-window/photo/dispatch/before-after signatury. Staging screenshot audit: `output/playwright/landing-salon-operating-2026-06-11/staging/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png`; všech 5 viewportů má horizontální overflow `0`.
...
- Root landing page používá aktuální claim `Plnější kalendář bez marketplace provizí.`
- Landing má top navigaci `Produkt`, `Pro koho`, `Návody`, `Ceník`, `Bez marketplace`, `Bezpečnost`, `Ukázka`.
- Hero má salon-operating app frame s plochami `Kalendář`, `Rezervace`, `Klienti` a `Kanály`; interaktivní průchod zůstává na `/ukazka`.
