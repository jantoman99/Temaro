...
- Aktuální frontend/design směr: homepage drží koncept `Salonový den, ne dashboard`. Root landing používá claim `Méně zvonění. Plnější kalendář.`, velkou salonovou fotku, interaktivní booking overlay `salon-phone-card`, photo-led sekci `Provoz v obrazech`, jednodušší bento, produktový demo blok, manifest, ceník, bezpečnost a závěrečné CTA. Sekce `Scénáře` zůstává odstraněná jako duplicita.
...
- Historická Time Engine foto/motion vrstva používala `public/marketing/time-engine-*.webp`, animovaný převod zpráv do dne a produktové overlaye přes fotky provozů. Aktuální `#casovy-engine` je photo-led `Provoz v obrazech` se `salon-day-*.webp`.
...
- Aktuální photo-led landing pass 2026-06-11 nahrazuje operating-table pass v aktivním UI: hero je `photo-led-hero`, používá `salon-day-hero.webp`, `salonServiceTabs`, `Volno dnes`, `Bez provize z vašich klientů`, CTA `Vybrat pilot` a zachovaný interaktivní výběr termínu. Mobil/tablet ukazují fotku před CTA/proof blokem; `salon-phone-card` je explicitně `!absolute` a mobilní rezervace/časy jsou kompaktní vodorovný overlay. `Provozní stůl dne`, `operating-table-stage`, `dailyOperations`, `decisionStack`, `Den před Temarem`, `Den s Temarem`, `hero-before-after-stage`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` a `id="trust-bar"` se nesmí vrátit.
...
- Poslední lokální ověření 2026-06-11 15:24 CEST po photo-led passu: `npx vitest run tests/landing-polish.test.ts` prošel 30/30, `npx impeccable detect app components` prošel bez výstupu a `npm run check` prošel s 608 Vitest testy, migrations check, type-check, lint a produkční build. Screenshoty: `output/playwright/landing-photo-led-2026-06-11/home-1440-fold.png`, `home-1366-fold.png`, `home-768-fold.png`, `home-390-fold.png`, `home-375-fold.png` a odpovídající full-page verze; všech 5 viewportů je bez horizontálního overflow.
- Poslední staging ověření zatím zůstává 2026-06-11 14:16 CEST po historickém operating-table passu (`dfc56e0`, `dpl_3PsCFBPZt3Tv4CnnuCmy45DCLcYj`). Po aktuálním photo-led commitu je potřeba znovu ručně nasadit preview, aliasovat `https://rezervacni-system-dev.vercel.app` a ověřit `/api/health` + HTML signatury `Méně zvonění`, `Plnější kalendář`, `photo-led-hero`, `salon-day-hero`, `salon-phone-card`, `salon-proof-strip`; zároveň nesmí být přítomné `operating-table-stage`, `Provozní stůl dne`, `hero-before-after-stage`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`.
