...
- Aktuální frontend/design směr: homepage drží koncept `Čas jako materiál`, ale first fold je po passu 2026-06-11 přepsaný na `Provozní stůl dne`. Root landing používá claim `Rezervace drží den pohromadě.`, fonty Bricolage Grotesque / Instrument Sans / IBM Plex Mono, tokeny `porcelain/cobalt/apricot/mint`, interaktivní časový výběr slotu, time engine, bento grid, produktový demo blok, manifest, ceník, bezpečnost a závěrečné CTA. Sekce `Scénáře` zůstává odstraněná jako duplicita.
...
- Aktuální landing operating-table pass 2026-06-11 nahrazuje before/after WOW pass v aktivním UI: hero je `Provozní stůl dne` s `operating-table-stage`, `dailyOperations`, `decisionStack`, desktop `operations-inbox`, časový pás `shift-time-band` a panel `decision-stack-panel`. Hero obsahuje `Rezervace drží den pohromadě.`, `Dnes 08:00-18:00`, `Volné okno chráněné`, `Rizikový klient` a zachovaný interaktivní výběr termínu. `Den před Temarem`, `Den s Temarem`, `hero-before-after-stage`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` a `id="trust-bar"` se nesmí vrátit.
...
- Poslední lokální ověření 2026-06-11 14:08 CEST po operating-table passu: `npx vitest run tests/landing-polish.test.ts` prošel 29/29, `npx impeccable detect app components` prošel bez výstupu a `npm run check` prošel s 607 Vitest testy, migrations check, type-check, lint a produkční build. Screenshoty: `output/playwright/landing-redesign-2026-06-11/implementation-pass-3/home-1440-pass3.png`, `home-1366-pass3.png`, `home-768-pass3.png`, `home-390-pass3.png`, `home-375-pass3.png`; všech 5 viewportů je bez horizontálního overflow.
- Poslední staging ověření 2026-06-11 14:16 CEST po operating-table passu: commit `dfc56e0` je na `origin/dev`, preview deploy `dpl_3PsCFBPZt3Tv4CnnuCmy45DCLcYj` je aliasovaný na `https://rezervacni-system-dev.vercel.app`. Ověřeno `/api/health=ok`, HTML obsahuje `Provozní stůl dne`, `Rezervace drží den pohromadě`, `operating-table-stage`, `Volné okno chráněné`, `Rizikový klient`, `Nastavit stejný den` a staré `hero-before-after-stage`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip`, `id="trust-bar"` se nevrací.
