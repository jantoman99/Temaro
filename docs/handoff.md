...
- Aktuální frontend/design směr: homepage drží koncept `Temaro Salon Operating`. Root landing používá claim `Plnější kalendář bez marketplace provizí.`, salonový code-native app rám `salon-command-frame`, oborový přepínač `industry-switcher`, desktopový klientský telefon `booking-phone-preview` a produktové proof sekce `salon-proof-board` / `marketplace-compare-panel`. Směr navazuje na Salona strukturou oborů, ceny a salonového kontextu, ale diferencuje Temaro přes vlastní klienty, nulové marketplace provize a app-only pohledy bez lifestyle/fake obrázků.
...
- Historická Time Engine/photo-led vrstva používala `public/marketing/time-engine-*.webp`, později `salon-day-*.webp` a product-window desku `product-proof-gallery product-proof-board`. Aktuální root landing už `#casovy-engine` nepoužívá; aktivní produktový důkaz je `#produktovy-dukaz` / `salon-proof-board`.
...
- Aktuální salon-operating landing pass 2026-06-11 nahrazuje product-window/photo-led/operating-table pass v aktivním UI: hero je `salon-operating-hero`, používá oborové profily Barber/Kosmetika/Kadeřnictví/Masáže, app rám `salon-command-frame`, CTA `Registrovat salon` a `Spustit ukázku`. Navazující sekce jsou `#pro-koho`, `#produktovy-dukaz`, `#cenik`, `#bez-marketplace`, `#jak-to-funguje` a `#bezpecnost`. `product-window-hero`, `product-window-frame`, `product-proof-gallery`, `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-hero.webp`, `Provozní stůl dne`, `operating-table-stage`, `Den před Temarem`, `Den s Temarem`, `hero-before-after-stage`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` a `id="trust-bar"` se nesmí vrátit.
...
- Poslední lokální ověření 2026-06-11 18:55 CEST po salon-operating passu: `npx vitest run tests/landing-polish.test.ts` prošel 20/20, `npx impeccable detect app components` prošel s exit 0 bez výstupu, `npm run lint` prošel a `npm run check` prošel s 598 Vitest testy, migrations check, type-check, lint a produkční build. Screenshoty: `output/playwright/landing-salon-operating-2026-06-11/final/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png`; všech 5 viewportů má horizontální overflow `0`, `salon-operating-hero=true`, staré photo/dispatch signatury `false`.
- Poslední staging ověření 2026-06-11 23:51 CEST po salon-operating passu: commit `f834477` je na `origin/dev`, preview deploy `dpl_8ozzwSbGcdG2XKRP9Ku3KJssKgFZ` je aliasovaný na `https://rezervacni-system-dev.vercel.app`. Ověřeno `/api/health=ok`, HTML signatury `Plnější kalendář bez marketplace provizí`, `salon-operating-hero`, `salon-command-frame`, `industry-switcher`, `marketplace-free-proof`, `salon-proof-board`, `marketplace-compare-panel`; staré product-window/photo/dispatch/before-after signatury nejsou přítomné. Staging screenshot audit je v `output/playwright/landing-salon-operating-2026-06-11/staging/` a potvrdil overflow `0` na 1440, 1366, 768, 390 a 375px.
