...
- Aktuální frontend/design směr: homepage drží koncept `Temaro Product Window`. Root landing používá claim `Rezervace, které vidíte hned v kalendáři.`, dominantní code-native app rám `product-window-frame`, klientský telefon `booking-phone-preview` a produktovou desku `product-proof-board` s týmovým kalendářem, veřejnou rezervací a zákaznickým účtem. Root landing už nemá lifestyle/AI salonové fotky, `photo-led-hero`, `salon-phone-card` ani `salon-day-*` obrázky; sekce `Scénáře` zůstává odstraněná jako duplicita.
...
- Historická Time Engine/photo-led vrstva používala `public/marketing/time-engine-*.webp` a později `salon-day-*.webp`. Aktuální `#casovy-engine` je produktová proof deska `product-proof-gallery product-proof-board`, ne galerie fotek provozů.
...
- Aktuální product-window landing pass 2026-06-11 nahrazuje photo-led i operating-table pass v aktivním UI: hero je `product-window-hero`, používá `productSurfaces`, `SurfacePanel`, `BookingPhonePreview`, app rám `product-window-frame`, CTA `Začít zdarma` a `Spustit produktovou ukázku`. Navazující `#casovy-engine` obsahuje `product-proof-board` s reálnými app pohledy. `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-hero.webp`, `Provozní stůl dne`, `operating-table-stage`, `dailyOperations`, `decisionStack`, `Den před Temarem`, `Den s Temarem`, `hero-before-after-stage`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` a `id="trust-bar"` se nesmí vrátit.
...
- Poslední lokální ověření 2026-06-11 16:48 CEST po product-window passu: `npx vitest run tests/landing-polish.test.ts` prošel 30/30, `npx impeccable detect app components` prošel s exit 0 bez výstupu a `npm run check` prošel s 608 Vitest testy, migrations check, type-check, lint a produkční build. Screenshoty: `output/playwright/landing-product-window-2026-06-11/pass6/home-1440.png`, `home-1366.png`, `home-768.png`, `home-390.png`, `home-375.png`; všech 5 viewportů má horizontální overflow `0`, `product-proof-board=true`, staré photo/dispatch signatury `false`.
- Poslední staging ověření 2026-06-11 17:01 CEST po product-window passu: commit `3c8fd0c` je na `origin/dev`, preview deploy `dpl_8HRQBqTehprzprf6p4zroA66NPjB` je aliasovaný na `https://rezervacni-system-dev.vercel.app`. Ověřeno `/api/health=ok` a HTML signatury `Rezervace, které vidíte hned v kalendáři`, `product-window-hero`, `product-window-frame`, `booking-phone-preview`, `product-proof-gallery`, `product-proof-board`, `Co klient a tým skutečně uvidí`; není přítomné `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-*.webp`, `Den před Temarem`, `Den s Temarem`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` ani `id="trust-bar"`.
