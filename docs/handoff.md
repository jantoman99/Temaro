...
- Staging deploy pro salon-operating pass zatím čeká na commit/push a ruční Vercel preview deploy. Po deployi ověřit `/api/health=ok` a HTML signatury `Plnější kalendář bez marketplace provizí`, `salon-operating-hero`, `salon-command-frame`, `industry-switcher`, `marketplace-free-proof`, `salon-proof-board`, `marketplace-compare-panel`; staré product-window/photo/dispatch/before-after signatury nesmí být přítomné.
