...
- Aktuální frontend/design směr: homepage drží koncept `Temaro Time Atelier` / `Ateliér času Temaro`. Root landing používá claim `Váš kalendář má mít vlastní rytmus.`, class `temaro-time-atelier-hero`, screenshot-led produktové kino `temaro-cinema-frame`, časovou linku `temaro-time-thread`, animované slovo v H1, count-up proof čísla a reálné Temaro screenshoty z `public/marketing/product/`.
...
- Historická Time Engine/photo-led vrstva používala `public/marketing/time-engine-*.webp`, později `salon-day-*.webp`, product-window desku a salon-operating code-native frame. Aktuální root landing už `#casovy-engine`, `product-window-*` ani `salon-operating-*` nepoužívá; aktivní produktový důkaz je `#produktovy-dukaz` / `temaro-screenshot-gallery`.
...
- Aktuální Time Atelier landing pass 2026-06-11 nahrazuje salon-operating/product-window/photo-led/operating-table pass v aktivním UI: hero je `temaro-time-atelier-hero`, používá `Ateliér času Temaro`, reálné screenshoty `temaro-product-app-screen.jpg`, `temaro-client-booking-mobile.jpg`, `temaro-business-discovery.jpg`, CTA `Registrovat salon` a `Spustit ukázku`. Navazující sekce jsou `#pro-koho`, `#produktovy-dukaz`, `#cenik`, `#bez-marketplace`, `#jak-to-funguje` a `#bezpecnost`. `salon-operating-hero`, `salon-command-frame`, `product-window-hero`, `product-window-frame`, `product-proof-gallery`, `photo-led-hero`, `salon-phone-card`, `/marketing/salon-day-hero.webp`, `Provozní stůl dne`, `operating-table-stage`, `Den před Temarem`, `Den s Temarem`, `hero-before-after-stage`, `dispatch-day-rail`, `mobile-day-rail`, `dispatch-proof-strip` a `id="trust-bar"` se nesmí vrátit.
...
- Staging pro Time Atelier pass zatím čeká na commit/push/deploy. Poslední staging před tímto passem je `dpl_8ozzwSbGcdG2XKRP9Ku3KJssKgFZ` na `https://rezervacni-system-dev.vercel.app` se starším salon-operating obsahem; po pushi je potřeba ověřit `/api/health`, HTML signatury nového směru a screenshoty 1440/1366/768/390/375.
